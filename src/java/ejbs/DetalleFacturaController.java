...
import ejbs.util.JsfUtil;
import ejbs.util.PaginationHelper;
...
public class DetalleFacturaController implements Serializable {
    ...
    @EJB
    private ejbs.DetalleFacturaFacade ejbFacade;
    ...
    public DetalleFactura getSelected() {
        if (current == null) {
            ...
            current.setDetalleFacturaPK(new Clases.DetalleFacturaPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getDetalleFacturaPK().setIdPedido(current.getPedido().getIdPedido());
            current.getDetalleFacturaPK().setNumFactura(current.getFactura().getNumFactura());
    ...
    public DetalleFactura getDetalleFactura(Clases.DetalleFacturaPK id) {
    ...
    public static class DetalleFacturaControllerConverter implements Converter {
        ...
        Clases.DetalleFacturaPK getKey(String value) {
            Clases.DetalleFacturaPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new Clases.DetalleFacturaPK();
            key.setNumFactura(Integer.parseInt(values[0]));
            key.setIdPedido(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(Clases.DetalleFacturaPK value) {
